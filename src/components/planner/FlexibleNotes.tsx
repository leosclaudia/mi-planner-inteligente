...
type FlexInkTool="off"|"pen"|"eraser";
type FlexInkSettings={tool:FlexInkTool;color:string;width:number};
const flexInkKey=(id:string)=>`planner-flex-ink-${id}-v1`;
...
export function FlexibleNotes(){
 ...
 const inkDrawingRef=useRef<{noteId:string;last:{x:number;y:number}|null}|null>(null);
 const [flexInk,setFlexInk]=useState<FlexInkSettings>({tool:"off",color:localStorage.getItem("planner-ink-color")||"#65475F",width:Number(localStorage.getItem("planner-ink-width")||4)});
 useEffect(()=>{
 const loaded=read();
 ...
 const sizeFlexInkCanvas=(id:string,c:HTMLCanvasElement|null)=>{
  ...
  const cssW=Math.max(1,host.clientWidth),cssH=Math.max(120,host.scrollHeight);
  const dpr=Math.max(1,window.devicePixelRatio||1),w=Math.round(cssW*dpr),h=Math.round(cssH*dpr);
  if(c.width===w&&c.height===h)return;
  const old=c.width?c.toDataURL("image/png"):localStorage.getItem(flexInkKey(id));
  const oldW=c.width,oldH=c.height;c.width=w;c.height=h;c.style.width=`${cssW}px`;c.style.height=`${cssH}px`;
  if(old){const img=new Image();img.onload=()=>{const ctx=c.getContext("2d");if(!ctx)return;ctx.clearRect(0,0,w,h);if(oldW&&oldH)ctx.drawImage(img,0,0,oldW,oldH);else ctx.drawImage(img,0,0)};img.src=old}
 };
 const flexInkPos=(id:string,e:React.PointerEvent<HTMLCanvasElement>)=>{const c=inkCanvasRefs.current[id]!;const r=c.getBoundingClientRect();return{x:(e.clientX-r.left)*(c.width/r.width),y:(e.clientY-r.top)*(c.height/r.height)}};
 const startFlexInk=(id:string,e:React.PointerEvent<HTMLCanvasElement>)=>{if(flexInk.tool==="off")return;markActive(id);const c=inkCanvasRefs.current[id];if(!c)return;inkDrawingRef.current={noteId:id,last:flexInkPos(id,e)};c.setPointerCapture?.(e.pointerId)};
 const moveFlexInk=(id:string,e:React.PointerEvent<HTMLCanvasElement>)=>{const d=inkDrawingRef.current,c=inkCanvasRefs.current[id];if(!d||d.noteId!==id||!d.last||!c||flexInk.tool==="off")return;const p=flexInkPos(id,e),ctx=c.getContext("2d");if(!ctx)return;const scale=c.width/c.getBoundingClientRect().width;ctx.lineCap="round";ctx.lineJoin="round";ctx.lineWidth=(flexInk.tool==="eraser"?Math.max(14,flexInk.width*3):flexInk.width)*scale;ctx.globalCompositeOperation=flexInk.tool==="eraser"?"destination-out":"source-over";ctx.strokeStyle=flexInk.color;ctx.beginPath();ctx.moveTo(d.last.x,d.last.y);ctx.lineTo(p.x,p.y);ctx.stroke();d.last=p};
 const endFlexInk=(id:string)=>{if(inkDrawingRef.current?.noteId===id)inkDrawingRef.current=null;const c=inkCanvasRefs.current[id];if(c)localStorage.setItem(flexInkKey(id),c.toDataURL("image/png"))};
 ...
 const remove=(id:string)=>{localStorage.removeItem(flexInkKey(id));if(transformRef.current?.noteId===id)transformRef.current=null;document.querySelectorAll<HTMLElement>(`[data-note-id="${id}"] .planner-flex-free-image`).forEach(x=>x.remove());document.querySelectorAll<HTMLElement>(".planner-flex-free-image").forEach(x=>x.dataset.selected="false");persist(notes.filter(n=>n.id!==id))};
 ...
 const rememberSelection=(id:string)=>{const text=document.querySelector<HTMLElement>(`[data-flex-note-body="${id}"] .planner-flex-text`),s=window.getSelection();if(!text||!s||!s.rangeCount||s.isCollapsed)return;const saved=saveSelectionSnapshot(id,text);if(saved){markActive(id);selectionRef.current=saved}};
