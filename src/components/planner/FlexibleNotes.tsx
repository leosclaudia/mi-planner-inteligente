...
export function FlexibleNotes(){
 ...
 const saveDom=(id:string,root:HTMLElement,rerender=true)=>{fitNoteHeight(root);const text=root.querySelector<HTMLElement>(".planner-flex-text")?.innerHTML||"";const images=Array.from(root.querySelectorAll<HTMLElement>(".planner-flex-free-image")).map(x=>{const clone=x.cloneNode(true) as HTMLElement;clone.dataset.selected="false";return clone.outerHTML}).join("");const html=text+images;if(rerender){patch(id,{html});return}const next=read().map(n=>n.id===id?{...n,html}:n);localStorage.setItem(KEY,JSON.stringify(next))};
