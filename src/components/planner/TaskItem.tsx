...
export function TaskItem({ task, onEdit }: { task: Task; onEdit: (t: Task) => void }) {
  ...
  return <li className="card-soft planner-print-item flex items-start gap-3 p-3">
    ...
    <div className="min-w-0 flex-1"><p className={cn("break-words font-semibold leading-snug", task.done && "text-muted-foreground line-through")}>{task.title}</p>{task.notes ? <p className="mt-0.5 text-sm text-muted-foreground">{task.notes}</p> : null}<div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">{section ? <SectionChip name={section.name} color={section.color} /> : null}{dateLabel ? <span className="rounded-full bg-muted px-2.5 py-0.5 font-medium">{dateLabel}</span> : null}<span className={cn("rounded-full px-2.5 py-0.5 font-medium", task.priority === "alta" ? "bg-terra-soft text-terra" : task.priority === "media" ? "bg-sun-soft text-sun" : "bg-muted")}>{priority}</span></div></div>
    <div className="flex shrink-0 flex-col gap-1 print:hidden"><Button size="icon" variant="ghost" aria-label={lang === "en" ? "Print only this task" : "Imprimir solo esta tarea"} title={lang === "en" ? "Print only" : "Imprimir solo"} onClick={printOnly}><Printer className="h-4 w-4" /></Button><Button size="icon" variant="ghost" aria-label={t("Editar tarea")} onClick={() => onEdit(task)}><Pencil className="h-4 w-4" /></Button><Button size="icon" variant="ghost" aria-label={t("Eliminar tarea")} onClick={() => removeTask(task.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div>
  </li>;
}
