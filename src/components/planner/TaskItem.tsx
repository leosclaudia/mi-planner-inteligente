...
export function TaskItem({ task, onEdit }: { task: Task; onEdit: (t: Task) => void }) {
  ...
  return (
    <li className="card-soft flex items-start gap-3 p-3">
      <button
        aria-label={task.done ? "Marcar como pendiente" : "Completar tarea"}
        onClick={() => toggleTask(task.id)}
        className={cn(
          "mt-0.5 grid h-10 w-10 shrink-0 place-items-center rounded-full border-2 transition-colors",
          task.done
      ...
      <div className="min-w-0 flex-1">
        ...
        {task.notes ? (
          <p className="mt-0.5 line-clamp-2 text-sm text-muted-foreground">{task.notes}</p>
        ) : null}
        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          {section ? <SectionChip name={section.name} color={section.color} /> : null}
          {task.date ? (
            <span className="rounded-full bg-muted px-2.5 py-0.5 font-medium">
              {fmt(new Date(`${task.date}T00:00:00`), "d MMM")}
            </span>
          ) : null}
          <span
            className={cn(
