...
function CalendarWeekCard({d,list,dateFmt,tx,setCursor,setView}:any){const key=KEY(d);const[bg]=useCardBackground(`calendar-week:${key}`,isToday(d)?"#FFF1E6":"#FFFFFF");return <div style={{backgroundColor:bg}} className={cn("relative min-h-24 rounded-xl border p-3 text-left transition-colors",isToday(d)?"border-primary":"border-border")}><CardBackgroundPicker storageKey={`calendar-week:${key}`} className="absolute right-2 top-2"/><button type="button" onClick={()=>{setCursor(d);setView("dia")}} className="w-full pr-10 text-left"><div className="flex items-start justify-between gap-2"><div><div className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{dateFmt(d,{weekday:"short"})}</div><div className="mt-1 text-lg font-bold">{d.getDate()}</div></div><span className="rounded-full bg-muted px-2 py-1 text-[10px] font-bold">{list.length}</span></div><div className="mt-2 space-y-1">{list.slice(0,2).map((task:any)=><div key={task.id} className={cn("truncate text-xs",task.done&&"line-through text-muted-foreground")}>{task.title}</div>)}{list.length>2?<div className="text-[10px] text-muted-foreground">+{list.length-2} {tx("más","more")}</div>:null}{list.length===0?<div className="text-[10px] text-muted-foreground">{tx("Sin tareas","No tasks")}</div>:null}</div></button></div>}
function CalendarMonthCard({d,list,selected,cursor,setCursor,setView}:any){const key=KEY(d);const[bg]=useCardBackground(`calendar-day:${key}`,selected?"#FFF1E6":"#FFFFFF");return <div style={{backgroundColor:bg}} className={cn("relative aspect-square rounded-lg border",isToday(d)?"border-primary":"border-transparent",isSameMonth(d,cursor)?"text-foreground":"text-muted-foreground/50")}><CardBackgroundPicker storageKey={`calendar-day:${key}`} className="absolute right-0 top-0 scale-75 origin-top-right"/><button onClick={()=>{setCursor(d);setView("dia")}} className="flex h-full w-full flex-col items-center justify-center text-sm">{d.getDate()}{list.length>0?<span className="mt-0.5 rounded-full bg-primary px-1.5 text-[9px] font-bold leading-4 text-primary-foreground">{list.length}</span>:null}</button></div>}
