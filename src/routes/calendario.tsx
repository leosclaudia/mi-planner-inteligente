...
function CalendarioPage() {
  const { state } = usePlanner(); const { t, lang } = useLanguage();
  const [view, setView] = useState<View>("dia"); const [cursor, setCursor] = useState(() => new Date()); const [open, setOpen] = useState(false); const [editing, setEditing] = useState<Task | null>(null);
  const locale = lang === "en" ? "en-US" : "es-AR";
  const dateFmt = (date: Date, options: Intl.DateTimeFormatOptions) => new Intl.DateTimeFormat(locale, options).format(date);
  const byDay = useMemo(() => { const map: Record<string, Task[]> = {}; for (const task of state.tasks) { if (!task.date) continue; (map[task.date] ??= []).push(task); } return map; }, [state.tasks]);
  ...
  return <PageShell title={t("Calendario")} subtitle={state.settings.plannerName} action={<Button className="h-11" onClick={() => { setEditing(null); setOpen(true); }}><Plus className="h-4 w-4" /> {t("Tarea")}</Button>}>
    ...
    <div className="mt-2 text-center"><button className="text-sm font-semibold text-primary" onClick={() => setCursor(new Date())}>{t("Ir a hoy")}</button></div>
    {view === "dia" ? <DayList tasks={byDay[KEY(cursor)] ?? []} onEdit={onEdit} emptyText={t("Sin tareas para este día.")} /> : view === "semana" ? <div className="mt-4 space-y-4">{weekDays(cursor).map((d) => <div key={KEY(d)}><div className="flex items-center gap-2"><h2 className={cn("text-base font-bold capitalize", isToday(d) && "text-primary")}>{dateFmt(d, { weekday: "long", day: "numeric" })}</h2><span className="text-xs text-muted-foreground">{(byDay[KEY(d)] ?? []).length} {lang === "en" ? "tasks" : "tareas"}</span></div><DayList tasks={byDay[KEY(d)] ?? []} onEdit={onEdit} compact emptyText={t("Sin tareas para este día.")} /></div>)}</div> : <div className="mt-4"><div className="grid grid-cols-7 gap-1 text-center text-xs font-semibold text-muted-foreground">{(lang === "en" ? ["M","T","W","T","F","S","S"] : ["L","M","M","J","V","S","D"]).map((d, i) => <span key={i}>{d}</span>)}</div><div className="mt-1 grid grid-cols-7 gap-1">{monthGrid(cursor).map((d) => { const list = byDay[KEY(d)] ?? []; const selected = isSameDay(d, cursor); return <button key={KEY(d)} onClick={() => { setCursor(d); setView("dia"); }} className={cn("flex aspect-square flex-col items-center justify-center rounded-xl border border-transparent text-sm", isSameMonth(d, cursor) ? "text-foreground" : "text-muted-foreground/50", isToday(d) && "border-primary font-bold", selected && "bg-terra-soft")}>{d.getDate()}{list.length > 0 ? <span className="mt-0.5 h-1.5 w-1.5 rounded-full bg-primary" /> : null}</button>; })}</div></div>}
    <TaskDialog open={open} onOpenChange={setOpen} task={editing} defaultDate={editing ? undefined : KEY(cursor)} />
  </PageShell>;
}

function DayList({ tasks, onEdit, compact, emptyText }: { tasks: Task[]; onEdit: (t: Task) => void; compact?: boolean; emptyText: string }) {
  if (tasks.length === 0) return <p className={cn("card-soft p-4 text-sm text-muted-foreground", compact ? "mt-2" : "mt-4")}>{emptyText}</p>;
