...
function NotasPage() {
  ...
  const restoreSelection = () => { const r = selectionRef.current; if (!r) return; const s = window.getSelection(); s?.removeAllRanges(); s?.addRange(r); };
  ...
  const applySize = (px: number) => {
    ...
    refs.current[active]?.querySelectorAll('font[size="7"]').forEach(node => { const el=node as HTMLElement; el.removeAttribute("size"); el.style.fontSize=`${px}px`; });
  ...
  const addImage = (file?: File) => {
    ...
    r.onload = () => { focus(); document.execCommand("insertHTML", false, `<img src="${r.result}" alt="Imagen" draggable="true" style="display:block;max-width:90%;height:auto;margin:12px auto;border-radius:14px;box-shadow:0 4px 14px rgba(47,43,41,.10);" />`); rememberSelection(); saveBox(active); };
  ...
  const restoreDrawing = (data?: string) => {
    const canvas=canvasRefs.current[active],ctx=canvas?.getContext("2d"); if(!canvas||!ctx)return;
    ctx.clearRect(0,0,canvas.width,canvas.height);
    if(!data){ localStorage.removeItem(drawKey(active)); return; }
    const img=new Image(); img.onload=()=>ctx.drawImage(img,0,0,canvas.width,canvas.height); img.src=data; localStorage.setItem(drawKey(active),data);
  };
  ...
  const clearDrawing = () => {
    ...
    if(canvas&&ctx){ ctx.clearRect(0,0,canvas.width,canvas.height); localStorage.removeItem(drawKey(active)); drawHistory.current[active]=[]; drawFuture.current[active]=[]; }
  };

  const prepareDrawingForOcr = (source: HTMLCanvasElement) => {
    const sourceCtx = source.getContext("2d"); if (!sourceCtx) return null;
  ...
  const handwritingToText = async () => {
    const canvas=canvasRefs.current[active]; if(!canvas)return;
    const prepared=prepareDrawingForOcr(canvas);
    if(!prepared){setOcrStatus("Primero escribí algo a mano dentro de la caja.");return;}
    setRecognizing(true);setOcrStatus("Reconociendo tu escritura… La primera vez puede tardar unos segundos.");
    try {
      ...
      if(!text){setOcrStatus("No pude reconocer esa escritura. Probá con letras un poco más separadas y claras.");return;}
      const el=refs.current[active];
      if(el){const before=el.innerText.trim();el.append(document.createTextNode(`${before?" ":""}${text}`));saveBox(active);}
      clearDrawing();setPanel(null);setEraser(false);setOcrStatus(`Convertido a texto: “${text}”`);
    } catch(error) {
      console.error(error);setOcrStatus("No se pudo cargar el reconocedor. Revisá la conexión y volvé a intentar.");
  ...
  return <PageShell title="Notas" subtitle="Tocá una hoja y empezá. Las herramientas aparecen cuando las necesitás.">
    <div className="sticky top-0 z-30 -mx-4 mb-4 border-y border-border bg-background/95 px-4 py-2 backdrop-blur">
      <div className="flex items-center justify-between gap-2">
        <div className="flex gap-1.5"><IconTool title="Negrita" onClick={()=>command("bold")}><Bold/></IconTool><IconTool title="Cursiva" onClick={()=>command("italic")}><Italic/></IconTool><IconTool title="Subrayar" onClick={()=>command("underline")}><Underline/></IconTool></div>
        <div className="flex gap-1.5"><IconTool title="Fuente y tamaño" active={panel==="fonts"} onClick={()=>setPanel(panel==="fonts"?null:"fonts")}><CaseSensitive/></IconTool><IconTool title="Imagen" onClick={()=>fileRef.current?.click()}><ImagePlus/></IconTool><IconTool title="Stickers" active={panel==="stickers"} onClick={()=>setPanel(panel==="stickers"?null:"stickers")}><SmilePlus/></IconTool><IconTool title="Color y resaltador" active={panel==="colors"} onClick={()=>setPanel(panel==="colors"?null:"colors")}><Palette/></IconTool><IconTool title="Escribir a mano" active={panel==="pen"} onClick={()=>setPanel(panel==="pen"?null:"pen")}><Pencil/></IconTool><IconTool title="Más" active={panel==="more"} onClick={()=>setPanel(panel==="more"?null:"more")}><MoreHorizontal/></IconTool></div>
      </div>

      {panel==="fonts"&&<div className="mt-2 rounded-2xl border bg-card p-3 shadow-sm"><div className="mb-3 flex items-center gap-2"><select className="h-11 min-w-0 flex-1 rounded-xl border bg-background px-3 text-base" onMouseDown={rememberSelection} onChange={e=>applyFont(e.target.value)} defaultValue=""><option value="" disabled>Elegir fuente</option>{fonts.map(f=><option key={f} value={f} style={{fontFamily:f}}>{f}</option>)}</select><select className="h-11 w-24 rounded-xl border bg-background px-2 text-base" value={fontSize} onMouseDown={rememberSelection} onChange={e=>applySize(Number(e.target.value))}>{FONT_SIZES.map(s=><option key={s} value={s}>{s} px</option>)}</select></div><button type="button" className="w-full rounded-xl border px-3 py-2 text-sm font-semibold" onMouseDown={e=>{e.preventDefault();rememberSelection();}} onClick={loadDeviceFonts}>Ver fuentes instaladas en este dispositivo</button>{fontStatus&&<p className="mt-2 text-xs text-muted-foreground">{fontStatus}</p>}</div>}

      {panel==="stickers"&&<div className="mt-2 rounded-2xl border bg-card p-3 shadow-sm"><div className="mb-2 flex gap-2 overflow-x-auto pb-1">{(Object.keys(STICKERS) as StickerGroup[]).map(g=><button key={g} className={`shrink-0 rounded-full border px-3 py-1.5 text-sm font-semibold ${stickerGroup===g?"bg-[#F3DDD6] border-[#D9A596]":"bg-background"}`} onClick={()=>{setStickerGroup(g);setStickerSearch("");}}>{g}</button>)}</div><label className="mb-2 flex h-10 items-center gap-2 rounded-xl border bg-background px-3"><Search className="h-4 w-4 text-muted-foreground"/><input className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="Buscar stickers" value={stickerSearch} onChange={e=>setStickerSearch(e.target.value)}/></label><div className="max-h-48 overflow-y-auto"><div className="grid grid-cols-8 gap-2 sm:grid-cols-12">{visibleStickers.map((s,i)=><button key={`${s}-${i}`} className="grid h-10 w-10 place-items-center rounded-xl text-xl hover:bg-muted" onMouseDown={e=>{e.preventDefault();rememberSelection();}} onClick={()=>insertSticker(s)}>{s}</button>)}</div></div></div>}

      {panel==="colors"&&<div className="mt-2 rounded-2xl border bg-card p-3 shadow-sm"><div className="mb-3 flex items-center gap-2"><Palette className="h-4 w-4"/><span className="text-sm font-bold">Color del texto</span></div><div className="flex flex-wrap gap-2">{TEXT_COLORS.map(c=><button key={c} title={c} className="h-9 w-9 rounded-full border-2 border-white shadow ring-1 ring-border" style={{backgroundColor:c}} onMouseDown={e=>{e.preventDefault();rememberSelection();}} onClick={()=>command("foreColor",c)}/>)}<label className="relative flex h-10 min-w-36 cursor-pointer items-center gap-2 rounded-xl border bg-background px-2 pr-3 text-xs font-semibold shadow-sm"><span className="h-7 w-7 shrink-0 rounded-lg border border-white shadow ring-1 ring-border" style={{background:"conic-gradient(#ef4444,#f59e0b,#eab308,#22c55e,#06b6d4,#3b82f6,#8b5cf6,#ec4899,#ef4444)"}}/><span>Todos los colores</span><input type="color" value={customTextColor} onMouseDown={rememberSelection} onChange={e=>{setCustomTextColor(e.target.value);command("foreColor",e.target.value);}} className="absolute inset-0 h-full w-full cursor-pointer opacity-0"/></label></div><div className="my-3 border-t"/><div className="mb-3 flex items-center gap-2"><Highlighter className="h-4 w-4"/><span className="text-sm font-bold">Resaltador</span></div><div className="flex flex-wrap gap-2">{HIGHLIGHT_COLORS.map(c=><button key={c} className="h-9 w-9 rounded-xl border shadow-sm" style={{backgroundColor:c}} onMouseDown={e=>{e.preventDefault();rememberSelection();}} onClick={()=>command("backColor",c)}/>)}<label className="relative flex h-10 min-w-36 cursor-pointer items-center gap-2 rounded-xl border bg-background px-2 pr-3 text-xs font-semibold shadow-sm"><span className="h-7 w-7 shrink-0 rounded-lg border border-white shadow ring-1 ring-border" style={{background:"linear-gradient(135deg,#fff59d,#f8bbd0,#e1bee7,#b3e5fc,#b2dfdb,#c8e6c9)"}}/><span>Otro resaltado</span><input type="color" value={customHighlightColor} onMouseDown={rememberSelection} onChange={e=>{setCustomHighlightColor(e.target.value);command("backColor",e.target.value);}} className="absolute inset-0 h-full w-full cursor-pointer opacity-0"/></label></div><div className="mt-3 flex flex-wrap gap-2"><button className="h-9 rounded-xl border px-3 text-sm" onMouseDown={e=>{e.preventDefault();rememberSelection();}} onClick={()=>command("foreColor","#2F2B29")}>Texto normal</button><button className="h-9 rounded-xl border px-3 text-sm" onMouseDown={e=>{e.preventDefault();rememberSelection();}} onClick={()=>command("backColor","transparent")}>Quitar resaltado</button><button className="h-9 rounded-xl border px-3 text-sm" onMouseDown={e=>{e.preventDefault();rememberSelection();}} onClick={()=>command("removeFormat")}>Quitar formato</button></div></div>}

      {panel==="pen"&&<div className="mt-2 rounded-2xl border bg-card p-3 shadow-sm"><div className="flex flex-wrap gap-2"><TextTool onClick={()=>setEraser(false)}><Pencil/>Manuscrito</TextTool><TextTool onClick={()=>setEraser(true)}><Eraser/>Goma</TextTool><TextTool onClick={undoDraw}><Undo2/></TextTool><TextTool onClick={redoDraw}><Redo2/></TextTool><select className="h-10 rounded-xl border bg-background px-3 text-sm" value={penWidth} onChange={e=>setPenWidth(Number(e.target.value))}><option value={2}>Fino</option><option value={3}>Medio</option><option value={5}>Grueso</option><option value={8}>Muy grueso</option></select><input type="color" value={penColor} onChange={e=>setPenColor(e.target.value)} className="h-10 w-12 rounded-xl border bg-card p-1" aria-label="Color del lápiz"/><TextTool onClick={clearDrawing}>Borrar dibujo</TextTool></div><button type="button" disabled={recognizing} onClick={handwritingToText} className="mt-2 h-11 w-full rounded-xl border border-[#D9A596] bg-[#F9E8E2] text-sm font-semibold disabled:opacity-60">{recognizing?"Reconociendo escritura…":"Convertir lo escrito a texto"}</button><p className="mt-2 text-xs text-muted-foreground">Este botón analiza lo que dibujaste y lo transforma en texto editable. La primera vez necesita conexión para cargar el reconocedor gratuito.</p></div>}
