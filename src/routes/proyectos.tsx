...
function ProyectosPage(){
 ...
 return <PageShell title={tx("Proyectos","Projects")} subtitle={tx(`${activeCount} en curso`,`${activeCount} active`)} action={<Button className="h-11" onClick={()=>openDialog(null)}><Plus className="h-4 w-4"/>{tx("Nuevo","New")}</Button>}>
   ...
   {projects.length===0?<p className="card-soft mt-4 p-4 text-sm text-muted-foreground">{tx("No hay proyectos para este filtro.","No projects for this filter.")}</p>:<ul className="mt-4 space-y-2">{projects.map(p=>{const tasks=state.tasks.filter(t=>t.projectId===p.id),done=tasks.filter(t=>t.done).length,section=p.sectionId?sections[p.sectionId]:undefined,progress=tasks.length?Math.round(done/tasks.length*100):0;return <li key={p.id} className="card-soft flex items-start gap-3 p-4"><div className="min-w-0 flex-1"><p className="break-words font-semibold">{p.name}</p>{p.description&&<p className="mt-0.5 text-sm text-muted-foreground">{p.description}</p>}<div className="mt-2 flex flex-wrap items-center gap-2">{section&&<SectionChip name={section.name} color={section.color}/>}<span className="rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium text-muted-foreground">{done}/{tasks.length} {tx("tareas","tasks")}</span><span className="text-xs font-semibold text-muted-foreground">{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{width:`${progress}%`}}/></div></div><div className="flex shrink-0 flex-col gap-1"><Button size="icon" variant="ghost" aria-label={tx("Editar proyecto","Edit project")} onClick={()=>openDialog(p)}><Pencil className="h-4 w-4"/></Button><Button size="icon" variant="ghost" aria-label={p.archived?tx("Restaurar proyecto","Restore project"):tx("Archivar proyecto","Archive project")} onClick={()=>updateProject(p.id,{archived:!p.archived})}>{p.archived?<ArchiveRestore className="h-4 w-4"/>:<Archive className="h-4 w-4"/>}</Button><Button size="icon" variant="ghost" aria-label={tx("Eliminar proyecto","Delete project")} onClick={()=>removeProject(p.id)}><Trash2 className="h-4 w-4 text-destructive"/></Button></div></li>})}</ul>}
